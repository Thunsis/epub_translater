...
def parse_arguments():
    ...
    parser.add_argument(
        "--no-verify-ssl",
        help="Disable SSL certificate verification for API calls (use if experiencing SSL certificate issues)",
        action="store_true",
        default=False
    )
    ...
    try:
        ...
        if api_needed:
            ...
            if args.no_verify_ssl:
                ...
                print("⚠️  Warning: SSL certificate verification is disabled. This may pose security risks.")
        ...
        epub_processor = EPUBProcessor(
            ...
            auto_extract_terms=False,  # Auto extraction is no longer used, using DeepSeek directly
        ...
        # -------------------------------------------------------------------
        # Phase 2: Terminology Generation with DeepSeek
        # -------------------------------------------------------------------
        if args.phase in ["terminology", "all"]:
            logger.info("=== Phase 2: Terminology Generation with DeepSeek ===")
            print("Starting terminology generation with DeepSeek API...")
            ...
            if args.phase == "terminology":
                ...
                # Explicitly enable the API for terminology phase when run directly
                if translator and hasattr(translator, 'enable_api'):
                    logger.info("Explicitly enabling API for terminology extraction phase")
                    translator.enable_api()
            
            # Now enhance terminology with DeepSeek
            terminology_generated = False
            try:
                print("Sending book structure to DeepSeek for terminology analysis...")
                # This method will use the DeepSeek API to generate terminology
                final_terminology = term_extractor.generate_terminology_with_deepseek()
                ...
                if final_terminology:
                    ...
                    logger.warning("No terminology was generated by DeepSeek")
                    print("No terminology was generated - translation will proceed with DeepSeek's implicit term handling")
            except Exception as e:
                logger.error(f"Error generating terminology: {e}")
                print(f"Error generating terminology: {e}")
                if args.phase == "terminology":
                    sys.exit(1)
                
                # Ask user if they want to continue when running all phases
                if args.phase == "all":
                    user_input = input("\nTerminology generation failed. Continue to translation phase anyway? (y/n): ")
            ...
            # Display results with appropriate success/failure message
            if terminology_generated:
                print(f"\n--- Phase 2 Complete: Terminology Generation ---")
                print(f"Processing time: {terminology_time:.2f} seconds")
                
                if args.phase == "terminology":
                    print("\nTerminology generation phase complete.")
                    print("You can now run Phase 3 (translation) to translate the content.")
                    return
            else:
                print(f"\n--- Phase 2 Failed: Terminology Generation ---")
                print(f"Processing time: {terminology_time:.2f} seconds")
                
                if args.phase == "terminology":
                    print("\nTerminology generation phase FAILED.")
                    print("Please fix the errors before proceeding to Phase 3.")
                    print("You may want to try again with the --no-verify-ssl flag if you're experiencing SSL issues.")
                    return
        ...
        if args.phase in ["translate", "all"]:
            ...
            if args.phase == "translate":
                ...
                # Explicitly enable the API for translate phase when run directly
                if translator and hasattr(translator, 'enable_api'):
                    logger.info("Explicitly enabling API for translation phase")
                    translator.enable_api()
        ...
        # Check if the error is an SSL certificate issue and provide a helpful message
        if "certificate verify failed" in error_msg or "SSL" in error_msg and "certificate" in error_msg:
            print(f"SSL Certificate Error: {error_msg}")
            print("\n----------------------------------------")
            print("This appears to be an SSL certificate verification issue.")
            print("To fix this, you can run the command again with the --no-verify-ssl flag:")
            print(f"\n    python {sys.argv[0]} {' '.join(sys.argv[1:])} --no-verify-ssl")
            print("\nCAUTION: Disabling SSL verification may pose security risks.")
            print("It's recommended to fix your certificate configuration if possible.")
            print("----------------------------------------\n")
        else:
